...
local function renameScreenGuis(newName)
    for _, gui in ipairs(CoreGui:GetDescendants()) do
        if gui:IsA("ScreenGui") and gui.Name == "ScreenGui" and gui:FindFirstChild("Frame") then
            gui.Name = newName
        end
    end
end
...
local EveryxHubUi = CoreGui:WaitForChild("EveryxHub")


local function renameScreenGuis2(newName)
    for _, gui in ipairs(EveryxHubUi:GetDescendants()) do
...
function getStoreButton()
    for i, gui in ipairs(game:GetService("Players").LocalPlayer.PlayerGui.Dialogue.Frame:GetDescendants()) do
        if gui:IsA("TextButton") and gui.Name == "3" then
            return gui
        end
    end
end
...
local function getItemsFromRS()
    for i, item in ipairs(game:GetService("ReplicatedStorage").Tools.Items:GetDescendants()) do
...
local ITSDropdown = Tabs.Main:AddDropdown("ITSDropdown", {
    Title = "Items to store",
    Description = "Select items to store.",
    Values = getItemsFromRS(),
    Multi = true,
    Default = "Bacon",
})


ITSDropdown:OnChanged(function(Value)
    local ToStoreValues = {}
    for Value, State in next, Value do
        table.insert(ToStoreValues, Value)
    end
    print("Debug " .. ToStoreValues)
end)
...
local ITSDropdown = Tabs.Misc:AddDropdown("ITSDropdown", {
    Title = "Items to store",
    Description = "Select items to store.",
    Values = getItemsFromRS(),
    Multi = true,
